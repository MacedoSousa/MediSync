...
const bundleId = "com.app.medsync";
...
export function getApiBaseUrl(): string {
  const webOrigin =
    ReactNative.Platform.OS === "web" && typeof window !== "undefined"
      ? window.location.origin
      : null;
  const runtime = Constants as unknown as {
    expoConfig?: { hostUri?: string };
    manifest?: { hostUri?: string; debuggerHost?: string };
    manifest2?: { extra?: { expoGo?: { debuggerHost?: string } } };
  };
  const nativeHostUri = selectNativeApiHost([
  ...
  return resolveApiBaseUrl({
    ...
    nativeHostUri,
    platform: ReactNative.Platform.OS as "android" | "ios" | "web",
    webOrigin,
  });
}
